    ...
    def create(self,provider:str):
        if provider==self.config.GEMINI:
            return GeminiProvider(
                ...
                embedding_model_id=self.config.GEMINI_EMBEDDING_MODEL_ID,
                embedding_dimention=self.config.GEMINI_EMBEDDING_DIMENSION
            )
        elif provider==self.config.GROQ:
            return GroqProvider(
                model_id=self.config.GROQ_MODEL_ID,
                api_key=self.config.GROQ_API_KEY,
                embedding_model_id=self.config.GROQ_EMBEDDING_MODEL_ID,
                embedding_dimention=self.config.GROQ_EMBEDDING_DIMENSION
            )
        else:
